...
fn embed_icon() {
  ...
  let windres_paths = vec![
    "/usr/bin/x86_64-w64-mingw32-windres", option_env!("WINDRES_EXE_PATH").unwrap_or("windres.exe"),
  ];
  ...
  // let ar_paths = vec![
  //   "/usr/bin/x86_64-w64-mingw32-ar" // TODO add search capability to btool?
  // ];
  // for p in ar_paths {
  //   if Path::new(p).exists() {
  //     res.set_ar_path(p);
  ...
  let ico_rel_path: PathBuf = ["..", "target", "icon.ico"].iter().collect();
